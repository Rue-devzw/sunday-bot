...
def calculate_age(dob_string):
    try:
        ...
        return None
# ... (All other helper and formatting functions remain the same)
...
def format_beginners_lesson(lesson):
    ...
    for section in lesson.get('lessonSections', []):
        if section.get('sectionType') == 'text':
            section_title, section_content = section.get('sectionTitle', 'Lesson Story'), section.get('sectionContent', '').strip()
            if section_content:
...
def handle_bot_logic(user_id, message_text):
    ...
    # Save user profile if it has changed
...
def handle_hymnbook_mode(user_id, user_profile, message_text):
    if 'hymnbook' not in user_profile:
        if message_text.lower() in HYMNBOOKS:
            selected_hymnbook = HYMNBOOKS[message_text.lower()]
...
def handle_bible_mode(user_id, user_profile, message_text):
    if 'bible_version_file' not in user_profile:
        if message_text.lower() in BIBLES:
            selected_bible = BIBLES[message_text.lower()]
