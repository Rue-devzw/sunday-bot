...
def check_registration_status(phone_number, sheet_name):
    ...
    try:
        ...
        # Column H (the 8th column) is assumed to hold the phone number.
...
def format_registration_details(data, camp_name):
    ...
    if volunteer_info == 'Yes':
        volunteer_info += f" ({data.get('Volunteer Department', '')})"
...
def handle_bot_logic(user_id, message_text):
    ...
    if mode == 'lessons':
        user_profile = handle_lessons_mode(user_id, user_profile, message_text_lower)
    elif mode == 'hymnbook':
        user_profile = handle_hymnbook_mode(user_id, user_profile, message_text)
    elif mode == 'bible':
        user_profile = handle_bible_mode(user_id, user_profile, message_text)
    elif mode == 'camp_registration':
        user_profile = handle_registration_mode(user_id, user_profile, message_text)
    elif mode == 'check_registration':
        user_profile = handle_check_status_mode(user_id, user_profile, message_text_lower)
    ...
    if 'class' not in user_profile:
        ...
        if message_text_lower.startswith('ask '):
            ...
            if not question:
                ...
                lesson_index, context = get_current_lesson_index(user_class), ""
                ...
                if lesson_file_name:
                    ...
                    if lessons_list and 0 <= lesson_index < len(lessons_list):
                        context = json.dumps(lessons_list[lesson_index])
                if not context:
                    send_whatsapp_message(user_id, "Sorry, I can't find this week's lesson material to answer questions about.")
                else:
                    send_whatsapp_message(user_id, get_ai_response(question, context))
...
def handle_bible_mode(user_id, user_profile, message_text):
    ...
    if 'bible_version_file' not in user_profile:
        ...
        verse_response = get_verse_from_db(message_text.strip(), db_filename)
...
def handle_registration_mode(user_id, user_profile, message_text):
    ...
    camp_name, camp_dates_text, arrival_eg, departure_eg = ("2025 Regional Youths Camp", "The camp runs from Aug 17 to Aug 24, 2025.", "Aug 17", "Aug 24") if reg_type == 'youths' else ("2025 Annual Camp", "The camp runs from Dec 7 to Dec 21, 2025.", "Dec 7", "Dec 21")
    ...
    if step == 'start':
        ...
        if not age: send_whatsapp_message(user_id, "That doesn't look right. Please enter your date of birth in DD/MM/YYYY format.")
            ...
            send_whatsapp_message(user_id, "What is your *gender*? (Type Male or Female)")
    elif step == 'awaiting_gender':
        if message_text.lower().strip() not in ['male', 'female']: send_whatsapp_message(user_id, "Please just answer with *Male* or *Female*.")
        else:
