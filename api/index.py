...
def handle_bot_logic(user_id, message_text):
    ...
    if user_profile.get('mode') == 'lessons':
        ...
        if message_text_lower == 'lesson':
            ...
            send_whatsapp_message(user_id, "You are in the *Lessons* section.\n\nType `lesson` to get your material, or `reset` to return to the main menu.")
    ...
    # --- PATH 2: USER IS IN 'HYMNBOOK' MODE ---
        ...
        if 'hymnbook' not in user_profile:
            if message_text_lower in HYMNBOOKS:
                ...
                user_profile['hymnbook'] = hymnbook_choice['file']
                users[user_id] = user_profile
                save_json_data(users, user_file)
...
@app.route('/')
def health_check():
    return "SundayBot with separated modes is running!", 200