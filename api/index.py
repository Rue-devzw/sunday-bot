...
ANCHOR_DATE = date(2024, 8, 21) # Updated anchor date
...
# --- 4. FORMATTING FUNCTIONS (ALL INCLUDED) ---
    ...
    for section in lesson.get('contentSections', []):
        ...
        if s_type == 'text': message += f"{section.get('text')}\n\n"
            ...
            if a_type == 'multipleChoice':
                ...
                for i, opt in enumerate(section.get('options', []), 1): message += f"{i}. {opt}\n"
                message += "\n"
...
# --- 5. AI "THINKING" FUNCTION using Groq ---
...
def handle_bot_logic(user_id, message_text):
    ...
    if user_profile.get('mode') == 'lessons':
        ...
        if message_text_lower == 'lesson':
            ...
            if not question: send_whatsapp_message(user_id, "Please type a question after `ask`."); return
            ...
            if not context: send_whatsapp_message(user_id, "Sorry, no lesson material found to answer questions about."); return
            ai_answer = get_ai_response(question, context); send_whatsapp_message(user_id, ai_answer)
        else:
