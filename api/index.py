...
def build_lesson_context(lesson, class_name):
    ...
    if "Beginners" in class_name: return format_beginners_lesson(lesson)
    elif "Answer" in class_name: return format_answer_lesson(lesson)
    elif "Search" in class_name: return format_search_lesson(lesson)
...
def format_beginners_lesson(lesson):
    ...
    bible_refs = ', '.join(bible_refs_list) if bible_refs_list else "Not specified"
    message = f"🖍️ *Beginners Lesson: {title}*\n\n_(Story from: {bible_refs})_\n\n"
    for section in lesson.get('lessonSections', []):
        if section.get('sectionType') == 'text': message += f"{section.get('sectionContent', 'No story available for this lesson.')}\n\n"
...
def get_ai_response(question, context):
    ...
    system_prompt = ("You are a friendly Sunday School assistant. Your purpose is to answer questions *strictly* based on the lesson material provided as context. "
                     "1. If the answer is in the text, answer it kindly and concisely. "
                     "2. If the answer is not in the text, you MUST politely state that the answer is not in the lesson material. Do not use external knowledge. "
                     "3. If the user asks a greeting or off-topic question, kindly redirect them to the lesson.")
